...
import { InviteAndSubscribe } from '~/backend/repositories/auth/InviteAndGiveProduct';
...
export const post = async (req: GetRequest, res: GetResponse) => {
  ...
  const user = await CheckForAccount({ supabase, data: { email: data.customer.email } });

  if (user) {
    if (user.is_subscribed && user.expiration_date && (new Date(user.expiration_date).getTime()) > Date.now()) {
