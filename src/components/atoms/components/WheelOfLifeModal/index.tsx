...
import { AnswersWrapper, AvatarWrapper, ClientName, FinishedDate, QuestionsText, TaskTitle, TitleWrapper, WheelWrapper } from './styles';
                ...
                <TaskTitle sx={{ left: '30%', }}>{selectedTask.title}</TaskTitle>
                    {
                        selectedTask.data?.map((question, index) => (
                            clientInputs.find((input) => question.id === input.id) ?
                                <Box key={question.id} sx={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>

                                    <QuestionsText >{question.title}</QuestionsText>
                                    <TextRating
                                        readOnly
                                        value={clientInputs[index].rating}
                                    />
                                </Box>
                                :
                                null
                    ...
                    <WheelWrapper>
                        <WheelOfLifeTemplate taskData={selectedTask.data} input={clientInputs} />
                    </WheelWrapper>
                </AnswersWrapper>
            </ModalDialogContent>
        </ModalComponent>
