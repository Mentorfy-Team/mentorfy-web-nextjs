...
const ProgressBar = ({data=[], input=[], activeid, onGoTo}:{data:DnDObject[]; input:MemberAreaTypes.UserInput[]; activeid:string; onGoTo:(id)=>void}) => {
    let count = 0;
    ...
    return (
        <Wrapper>
            {data.map((step, i) => (<div>
                <BundleWrapper>
                <CircleWrapper>
                    <CircleProgressBar value={getProgressByStep(step)} variant="determinate" size={25} thickness={5} />
                    { step.rows.length > 0 && <><Line /><Line /></>}
                </CircleWrapper>
                <TextWrapper>
                    <Title>{mock.title}</Title>
                    <ClassesNumber>{`${getDoneByStep(step)} de ${step.rows.length} aula${step.rows.length > 1 ?'s':''}`}</ClassesNumber>
                </TextWrapper>
            </BundleWrapper>
                {step.rows.map((task, j) => {
                    count++;
                    return(
                    <StepsWrapper onClick={()=>onGoTo(task.id)}>
                    <CircleWrapper>
                        <Line />
                        <Dot className={activeid === task.id ? 'active-background':''} sx={{
                            backgroundColor: getIsDone(task.id) ? '#38c284' : 'unset',
                        }} />
                        { j+1 !== step.rows.length ? <Line /> : <Box height={16} />}
                    </CircleWrapper>
                    <TextWrapper>
                        <ClassesNumber className={activeid === task.id ? 'active':''} sx={{
                            marginTop: '-0.1rem',
